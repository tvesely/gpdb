...
-- FIXME: only show results for our namespace here
...
--FIXME: add in dependencies for constraints for a partition table
...
--FIXME: doesn't quite work....only shows one table_name even though many deps...
DROP FUNCTION IF EXISTS show_constraints_and_deps();
CREATE FUNCTION show_constraints_and_deps() RETURNS TABLE(constraint_name name, index_name name, constraint_type char, deptype "char")
  LANGUAGE SQL STABLE STRICT AS $fn$
SELECT c.constraint_name, c.index_name, c.constraint_type, d.deptype
       FROM
         (SELECT table_name, constraint_name, CAST(index_name as name), constraint_type from constraints_and_indices()) as c
         RIGHT JOIN
          (SELECT depname, refname, classid, objid, objsubid, refclassid, refobjid, refobjsubid, deptype  from dependencies()) as d
...
SELECT partition_name, parent_name, root_name, constraint_name, index_name, constraint_type FROM partition_tables_show_all();
ALTER TABLE r SPLIT PARTITION r2 AT (6) INTO (PARTITION r2_split_l, PARTITION r2_split_r);  --FIXME: doesn't work!
SELECT partition_name, parent_name, root_name, constraint_name, index_name, constraint_type FROM partition_tables_show_all();
...
-- Previously existing constraints on a mid-level partition will inherit a
-- semantically identical constraint when it is added to the root partition
SELECT recreate_three_level_table();
ALTER TABLE r_1_prt_r1 ADD CONSTRAINT joinme UNIQUE(r_key,r_name);
SELECT partition_name, parent_name, root_name, constraint_name, index_name, constraint_type FROM partition_tables_show_all();
ALTER TABLE r ADD CONSTRAINT joinit UNIQUE(r_key,r_name);
-- The 'joinme' constraint now inherits from 'joinit', and only one constraint
-- should exist per partition
SELECT partition_name, parent_name, root_name, constraint_name, index_name, constraint_type FROM partition_tables_show_all();
-- Because 'joinme' now inherits from 'joinit', it can no longer be dropped
-- independently of 'joinit'
ALTER TABLE r_1_prt_r1 DROP CONSTRAINT joinme;
SELECT partition_name, parent_name, root_name, constraint_name, index_name, constraint_type FROM partition_tables_show_all();
-- Dropping joinit will cascade to all partitions
ALTER TABLE r DROP CONSTRAINT joinit;
SELECT partition_name, parent_name, root_name, constraint_name, index_name, constraint_type FROM partition_tables_show_all();
