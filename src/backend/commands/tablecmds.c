...
const char *reshuffle_sql = "(internal update command generated by alter table)";
...
ReshuffleRelationData(Relation rel)
{
	...
	for(i = 0; i < policy->nattrs; i++)
	{
		...
		coalExpr->args = list_make1(column);
		coalExpr->location = -1;

		res->val = (Node *) coalExpr;
		res->location = -1;
	...
	q = (Query *) linitial(rewritten);
	Assert(q->commandType == CMD_UPDATE);

	/* plan the query */
	planned_stmt = planner(q, 0, NULL);
	...
	/* Create a QueryDesc requesting no output */
	queryDesc = CreateQueryDesc(planned_stmt, pstrdup(reshuffle_sql),
								GetActiveSnapshot(), InvalidSnapshot,
 ...
 *
 * GPDB: This is only being used internally in DefineIndex and
 * cdb_exchange_part_constraints. The DDL has not been exposed to the end user.
 * This is the workhorse of ALTER INDEX ... ATTACH PARTITION in upstream, which
 * attaches parent indexes to their children.
 */
void
ATExecAttachPartitionIdx(List **wqueue, Relation parentIdx, AlterPartitionId *alterpartId, bool is_partition_exchange)
{
	...
	if (currParent != RelationGetRelid(parentIdx))
	{
		...
		/*
		 * Partition exchange attaches the index to its parent constraint before attaching
		 * partition to its parent, so skip this check
		 */
		if (!is_partition_exchange)
		{
