...
ATExecAttachPartitionIdx(List **wqueue, Relation parentIdx, AlterPartitionId *alterpartId, bool is_partition_exchange)
{
	...
	if (currParent != RelationGetRelid(parentIdx))
	{
		...
		// TODO: What to do here? Does the parent index need to be marked valid?
...
validatePartitionedIndex(Relation partedIdx, Relation partedTbl)
{
	...
	// TODO: What is the best way to assert here?
	...
	while ((inhTup = systable_getnext(scan)) != NULL)
	{
		...
		indTup = SearchSysCache1(INDEXRELID,
								 ObjectIdGetDatum(inh/Form->inhrelid));
		if (!indTup)
	...
	if (tuples == RelationGetPartitionDesc(partedTbl)->nparts)
	{
		...
		simple_heap_update(idxRel, &partedIdx->rd_indextuple->t_self, newtup);
		CatalogUpdateIndexes(idxRel, newtup);
